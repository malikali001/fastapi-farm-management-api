...
@router.get("/", response_model=list[schemas.healths.Health])
def read_healths(
...
@router.get("/{health_id}", response_model=schemas.healths.Health)
...
@router.post("/", response_model=schemas.healths.Health)
def create_health(
    health: schemas.healths.HealthCreate, db: Session = Depends(dependencies.get_db)
):
    return models.Health.create(db=db, health=health)


@router.put("/{health_id}", response_model=schemas.healths.Health)
def update_health(
    health_id: int,
    health: schemas.healths.HealthUpdate,
...
@router.delete("/{health_id}", response_model=schemas.healths.Health)
